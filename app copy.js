let fen = 'rbna1abnr/4k4/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/4K4/RNBA1ABNR w - - 0 1';
let board = [];

let pieceToImage = {
  'r': 'cho_cha',
  'n': 'cho_ma',
  'b': 'cho_sang',
  'a': 'cho_sa',
  'k': 'cho_gung',
  'c': 'cho_fo',
  'p': 'cho_jol',
  'R': 'han_cha',
  'N': 'han_ma',
  'B': 'han_sang',
  'A': 'han_sa',
  'K': 'han_gung',
  'C': 'han_fo',
  'P': 'han_b'
};

let images = {};
let bg;
let selectedPiece = null;

function preload() {
    bg = loadImage('JanggiBrown.png');
    for (let piece in pieceToImage) {
        images[piece] = loadImage(pieceToImage[piece] + '.png');
    }
    console.log(images);
}

function setup() {
  createCanvas(400, 400);
  noLoop();
  fen = fen.split(' ')[0];
  board = fen.split('/').map(row => {
    let newRow = [];
    for (let char of row) {
      if (isNaN(char)) {
        newRow.push(char);
      } else {
        for (let i = 0; i < parseInt(char); i++) {
          newRow.push(' ');
        }
      }
    }
    return newRow;
  });
  console.log(board);
}

function draw() {
  background(bg);
  let w = width / 9;
  let h = height / 10;
  
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 10; j++) {
      let piece = board[j][i];
      if (piece in images) {
        image(images[piece], i * w, j * h, w, h);
      }
    }
  }
  
  // 선택된 기물에 표시 추가
  if (selectedPiece) {
    stroke(255, 0, 0); // 빨간색 테두리
    strokeWeight(4);
    noFill();
    rect(selectedPiece.i * w, selectedPiece.j * h, w, h);
  }
}

function isSameTeam(piece1, piece2) {
  if (!piece1 || !piece2) return false;
  return (piece1.toLowerCase() === piece1 && piece2.toLowerCase() === piece2) ||
         (piece1.toUpperCase() === piece1 && piece2.toUpperCase() === piece2);
}

function isCannon(piece) {
  return piece === 'c' || piece === 'C';
}

function isValidMove(piece, from, to) {
  // 초 사와 초 궁의 유효한 위치
  const choSaGungPositions = [
    {i: 3, j: 0}, {i: 4, j: 0}, {i: 5, j: 0},
    {i: 3, j: 1}, {i: 4, j: 1}, {i: 5, j: 1},
    {i: 3, j: 2}, {i: 4, j: 2}, {i: 5, j: 2}
  ];
  
  // 한 사와 한 궁의 유효한 위치
  const hanSaGungPositions = [
    {i: 3, j: 7}, {i: 4, j: 7}, {i: 5, j: 7},
    {i: 3, j: 8}, {i: 4, j: 8}, {i: 5, j: 8},
    {i: 3, j: 9}, {i: 4, j: 9}, {i: 5, j: 9}
  ];
  
  if (piece === 'a' || piece === 'k') {
    return choSaGungPositions.some(pos => pos.i === to.i && pos.j === to.j) &&
           Math.abs(to.i - from.i) <= 1 && Math.abs(to.j - from.j) <= 1;
  } else if (piece === 'A' || piece === 'K') {
    return hanSaGungPositions.some(pos => pos.i === to.i && pos.j === to.j) &&
           Math.abs(to.i - from.i) <= 1 && Math.abs(to.j - from.j) <= 1;
  } else if (piece === 'p') {
    // 초 졸의 이동 규칙: 뒤로 이동 불가, 앞/좌/우로 한 칸 이동
    return (to.j === from.j + 1 && to.i === from.i) || // 앞으로 이동
           (to.j === from.j && Math.abs(to.i - from.i) === 1); // 좌우로 이동
  } else if (piece === 'P') {
    // 한 병의 이동 규칙: 뒤로 이동 불가, 앞/좌/우로 한 칸 이동
    return (to.j === from.j - 1 && to.i === from.i) || // 앞으로 이동
           (to.j === from.j && Math.abs(to.i - from.i) === 1); // 좌우로 이동
  }
  
  return true;
}

function mousePressed() {
  let i = floor(mouseX / (width / 9));
  let j = floor(mouseY / (height / 10));
  
  if (selectedPiece) {
    let selectedPieceType = board[selectedPiece.j][selectedPiece.i];
    let targetPieceType = board[j][i];
    
    // 같은 팀인지 확인
    if (selectedPiece.i === i && selectedPiece.j === j) {
      // 이미 선택된 기물을 다시 클릭한 경우
      selectedPiece = null;
      redraw();
    } else if (targetPieceType === ' ' || (!isSameTeam(selectedPieceType, targetPieceType) && !(isCannon(selectedPieceType) && isCannon(targetPieceType)))) {
      if (isValidMove(selectedPieceType, selectedPiece, {i, j})) {
        board[j][i] = selectedPieceType;
        board[selectedPiece.j][selectedPiece.i] = ' ';
        selectedPiece = null;
        redraw();
      }
    }
  } else if (board[j][i] in images) {
    selectedPiece = {i, j};
    redraw(); // 선택된 기물을 표시하기 위해 화면을 다시 그립니다.
  }
  
  console.log(selectedPiece);
  console.log(board);
}
